...
def ward_tra(src, ward: str, lat: float, lon: float) -> dict:
    ...
    min_cells = max(1, int(round(MIN_REFUGE_HA * 10_000 / cell_area_m2)))
    lab, n = label(raw)
    if n == 0:
    ...
    counts = {CLASS_NAME[c]: int((cover == c).sum()) for c in sorted(REFUGE_CLASSES)
              if (cover == c).any()}
    return {
        "tra": round(float(decay.mean()), 4),
        "median_dist_m": round(float(np.median(core)), 1),
        "max_dist_m": round(float(core.max()), 1),
        "refuge_cell_fraction": round(float(refuge.mean()), 4),
        "refuge_patches": int(keep.size),
        "largest_patch_ha": round(float(sizes.max() * cell_area_m2 / 10_000), 2),
...
def main():
    ...
    print(f"  {'ward':<14}{'TRA':>8}{'med d':>9}{'max d':>9}{'refuge':>9}")
