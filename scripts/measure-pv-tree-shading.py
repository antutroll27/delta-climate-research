    ...
    with open(out, "w") as fh:
        json.dump({
            ...
            "canopy": {"source": canopy_mod.CHM_PREFIX, "version": "v2", "mae_m": CHM_MAE_M,
                       ...
                       "px_over_min_m": int((chm > CANOPY_MIN_M).sum()),
                       "reader_floor_note": "read_chm_grid returns a 1 m floor over the whole box (the tile's zeros sit under its mask band); native tile 47.9% zeros in Ballygunge. Nothing below CANOPY_MIN_M enters the mask or the march, so only the count above it is a fingerprint.",
                       },
