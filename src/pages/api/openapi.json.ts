...
export const GET: APIRoute = () => new Response(JSON.stringify({
  ...
  paths: {
    ...
    '/api/stac/catalog.json': { get: { summary: 'STAC 1.0.0 static catalogue root', responses: { '200': json('STAC Catalog') } } },
    '/api/stac/collections/{id}.json': { get: { summary: 'STAC Collection', parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string', enum: ['ward-products'] } }], responses: { '200': json('STAC Collection') } } },
    '/api/stac/items/{id}.json': { get: { summary: 'STAC Item — one per ward x product (surface, canopy, buildings)', parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string' } }], responses: { '200': json('STAC Item', 'application/geo+json') } } },
    '/api/licence.json': { get: { summary: 'ODbL position for the geometry exports: which endpoints it covers and how each obligation is discharged', responses: { '200': json('licence statement') } } },
