...
function loadSurfaceMeta(): Promise<Record<string, SurfaceMetaEntry> | null> {
  surfaceMetaPromise ??= fetch('/heat-map/data/surface-meta.json')
    .then(r => (r.ok ? r.json() : null))
    .then(j => j?.wards ?? null)
    .catch(() => null);
...
export async function loadWardSurface(ward: string, signal?: AbortSignal): Promise<WardSurface> {
  const [inputs, meta, surface] = await Promise.all([
    loadInputs(), loadSurfaceMeta(), loadSurfaceRaster(ward, signal),
  ]);
  ...
  /* THE DC-URS SCALAR WINS WHERE THERE IS ONE, and that ordering is what keeps
     ...
     `record` is truthy, `measured` is never consulted, and every line below
     behaves exactly as it did before surface-meta.json was read at all. */
  const measured = record ? null : measuredMeans(meta?.[ward]);
  const means: SurfaceMeans = record
    ? { fvc: record.fvc.value, albedo: record.albedo.value }
    : measured ?? { fvc: 0, albedo: DEFAULT_ALBEDO };
  /* A texture with no level behind it is still refused — but "no level" now
     means neither a scalar nor a measurement, rather than "not in the Kolkata
     file". The `fvc: 0` below is the hot-biased fallback described at the top of
     this module; it is now only reachable when the ward has no evidence at all. */
